...
export default function SignUp() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await fetch('https://life-tag-backend-ahmedrashed2611-5674s-projects.vercel.app/api/signup', {
        method: 'POST',
  ...
  return (
    <div className="flex-1 flex flex-col p-6 items-center">
      ...
      <div className="flex w-full bg-gray-100 rounded-full p-1 mb-8">
        ...
        <Link to="/signin" className="flex-1 text-center py-2 rounded-full text-sm font-medium text-gray-500 hover:text-gray-800">
          Sign In
        </Link>
      </div>

      <form onSubmit={handleSignup} className="w-full flex-1 flex flex-col gap-4">
        <div>
